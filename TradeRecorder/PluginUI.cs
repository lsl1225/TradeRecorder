...
namespace TradeRecorder
{
  public unsafe class PluginUI : IDisposable
  {
    ...
    private static TextureWrap? GetIconStr(uint iconId, bool isHQ) {
      ...
      var icon = isHQ ? DalamudInterface.DataManager.GameData.GetHqIcon(iconId) : DalamudInterface.DataManager.GameData.GetIcon(iconId);
      if (icon != null)
      {
        return (TextureWrap?)DalamudInterface.TextureProvider.GetTexture(icon);
      }
      return null;
    }
  }
